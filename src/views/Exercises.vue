  	...
  	<div>
		...
		>
        	...
        	<div v-else>
				...
				<div class="py-2" v-if="exerciseStore.selectedOptions[index]" v-html="renderKaTeXContent(`$$ ${exerciseStore.selectedOptions[index]} $$`)"></div>
			</div>
        </div>
        <div 
        ...
        >
			<a 
				@click="explanationHandler(index)" 
				class="font-bold hover:cursor-pointer"
			>
				{{ explanation[index] ? 'Hide' : 'View' }} Solution:
			</a>
          	<ol>
				<li 
					class="pb-2" 
